% options_pub.outputDir=fullfile(pwd,'latex','033','html'); options_pub.showCode=true;
% publish(fullfile(pwd,'cal_report_033a2.m'),options_pub);
...
[NTC{2},ajuste{2},Args{2},Fraw,Fnew]=temp_coeff_raw(config_temp,sl_raw{Cal.n_inst},'outlier_flag',1,...
                                  'date_range',datenum(Cal.Date.cal_year-2,8,1));
...
[NTCx,ajustex,Argsx,Fraw,Forig]=temp_coeff_raw(config_temp,sl_raw{Cal.n_inst},'outlier_flag',1,'plots',0,...
                                'N_TC',TCorig(1:5)','date_range',datenum(Cal.Date.cal_year-2,8,1));
...
% Temperature Range
% Para que funcione asignamos la salida Fr en la llamada a la funci�n
...
else warning('No se est�n guardando los inputs de la funci�n!!')
end
...
[ETC_FILTER_CORRECTION,media_fi,fi,fi_avg]=filter_rep(Cal.brw_str{Cal.n_inst},...
                             'date_range',datenum(Cal.Date.cal_year-4,Cal.Date.cal_month+1,15),...
