...
set(gca,'FontSize',9); 
hline(0,'--k'); box on;
leg_b=Cal.brw_str(orden);
%legendflex(h,leg_b,'fontsize',8,'xscale',0.5,'anchor',[4 4],'buffer',[55 0]);
...
osc_lim=1500;
jo=(osc<osc_lim) ;
brw_ref=[2,10,11,14,15,17];
leg=[15,  2    10    11 14     17]
plot_b=find(ismember(orden,[2,10,11,14,15,17]))
close all
...
h1=errorbar(1:6,nanmean(rp(jo,plot_b)),nanstd(rp(jo,plot_b)),'bo','linewidth',3);
h2=errorbar(1:6,nanmean(rpsl(jo,plot_b)),nanstd(rpsl(jo,plot_b)),'rp','linewidth',3);
set(gca,'Xtick',1:6,'XtickLabel',Cal.brw_str(leg),'Layer','top'); box on;
%h=boxplot(100*matdiv(matadd(ref_sl(:,2:end),-ref(:,2)),ref(:,2)),Cal.brw_str(orden));

set(gca,'Ylim',[-5,2],'XLim',[0 6+1]); grid;
...
for i=plx
 ...
 set(lg,'FontSize',5); set(gca,'Ylim',[-5,5],'FontSize',8);
 set(gca,'Ytick',[-5:1:5])
 hline(0,':r');
