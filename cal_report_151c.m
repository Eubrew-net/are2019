% options_pub.outputDir=fullfile(pwd,'latex','151','html'); options_pub.showCode=true;
% close all; publish(fullfile(pwd,'cal_report_151c.m'),options_pub);
...
legend(gca,'inst','IZO#151','Location','NorthEast'); grid;
datetick('x',26,'KeepLimits','KeepTicks');
ozone_filter_analysis_mi(summary,Cal);
%% Ozone Calibration
% Reference Brewer #151
close all
...
% Esto podr?a ser una incoherencia: por ejemplo para el 072 sale 3179, pero mantenemos 3151.
...
%array2table(ozone_osc_sum,'VariableNames',str2name(label_),'Rownames',tags_(ozone_osc_sum(:,end))')
...
ozone_day_sum=round([m(:,1),m(:,2),s(:,2),n(:,4),m(:,8),s(:,8),100*(m(:,8)-m(:,2))./m(:,2)]*10)/10;
