...
% publish(fullfile(pwd,'cal_report_075a1.m'),options_pub);
...
try
    analyzeCZ(fullfile(Cal.path_root,['bdata',br],['HS*.',br]),...
                              'date_range',datenum(Cal.Date.cal_year,1,[Cal.Date.day0-300 Cal.Date.dayend]));
catch exception
      fprintf('Error: %s, brewer %s\n',exception.stack.name,Cal.brw_name{Cal.n_inst});
end
...
try
    [LRatPFHT Error]=analyzeCI(fullfile(Cal.path_root,['bdata',br],['CI*.',br]),...
                           'CI10019.075',...
                          'depuracion',1,'outlier_flag',1,...
                           'date_range',datenum(Cal.Date.cal_year,4,1));
                          %'date_range',datenum(Cal.Date.cal_year,1,[Cal.Date.day0 Cal.Date.dayend]));
catch exception
      fprintf('Error: %s, brewer %s\n',exception.stack.name,Cal.brw_name{Cal.n_inst});
end
