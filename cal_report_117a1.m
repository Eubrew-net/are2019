...
% publish(fullfile(pwd,'cal_report_117a1.m'),options_pub);
...
try
    analyzeCZ(fullfile(Cal.path_root,['bdata',br],['CZ*.',br]),...
                             'date_range',datenum(Cal.Date.cal_year-3,Cal.Date.cal_month+1,1));
catch exception
...
try
    analyzeCZ(fullfile(Cal.path_root,['bdata',br],['HL*.',br]),...
                              'date_range',datenum(Cal.Date.cal_year,1,[Cal.Date.day0-90 Cal.Date.dayend]));
catch exception
      fprintf('Error: %s, brewer %s\n',exception.stack.name,Cal.brw_name{Cal.n_inst});
end
%%
...
try
    [LRatPFHT Error]=analyzeCI(fullfile(Cal.path_root,['bdata',br],['CI*.',br]),...
                           fullfile(Cal.path_root,['bdata',br],'CI14117.117'),...
