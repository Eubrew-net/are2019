...
% publish(fullfile(pwd,'cal_report_117a2.m'),options_pub);
...
if ~isnan(sl_rw)
    ...
    [NTC{1},ajuste{1},Args{1},Fr]=temp_coeff_raw(config_temp,sl_rw,'outlier_flag',0,...
        'date_range',datenum(Cal.Date.cal_year,1,Cal.calibration_days{Cal.n_inst,1}([6 end])));
...
[ETC_FILTER_CORRECTION,media_fi,fi,fi_avg]=filter_rep(Cal.brw_str{Cal.n_inst},...
                             'date_range',datenum(Cal.Date.cal_year-2,1,1),...
